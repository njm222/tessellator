...
export default class VisualizerCanvas extends Vue {
  ...
  private mode1 (SpotifyAnalysisUtils: any) {
    ...
    const speed = (Date.now() + VisualizerCanvas.liveAudio.bassObject.bassEnergy + VisualizerCanvas.liveAudio.snareObject.snareEnergy) / (SpotifyAnalysisUtils.trackFeatures.tempo * SpotifyAnalysisUtils.trackFeatures.energy * SpotifyAnalysisUtils.trackFeatures.danceability * 30)
  ...
  private addGenerativeTorus (SpotifyAnalysisUtils: any) {
    ...
    const material = new THREE.PointsMaterial({ color: 0xFFF, size: Math.min(Math.max(segments / 3, 1.5), 2.5), map: VisualizerCanvas.circleSprite, alphaTest: 0.5, transparent: true })
